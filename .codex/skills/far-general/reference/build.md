...
- `test_Body.zip` / Body selection effect: exclude on every platform.
...
Web-specific rule: keep demo effects as ZIP archives in `public/effects/` and do **not** unzip them. Pass local ZIP URLs to the SDK, e.g. `new Effect("effects/Afro.zip")`. Do not pass a directory path such as `effects/Afro`; Vite may return the HTML fallback for that URL and WebAR will fail with a non-zip MIME type like `text/html`.

Use **Shared principles → Demo effects** only for selecting which ZIP files and icons to download. Ignore its unzip/double-nesting steps for Web.
...
#### 5. Build a stable camera and UI layout

When generating a Web demo or app UI, make the camera preview and controls visually stable before finishing:

- Render `Dom.render(player, container)` into a dedicated preview container with explicit responsive geometry: `position: relative`, `overflow: hidden`, `aspect-ratio` (usually `16 / 9`), `min-height`, and `width: 100%`.
- Constrain SDK-rendered children inside that container: target `canvas`, `video`, and direct render children with `width: 100%`, `height: 100%`, and `object-fit: cover` (or `contain` if the product requires no crop). Do not let the intrinsic canvas/video size resize the page.
- Keep controls and effect grids outside the preview element. Use a parent grid/flex shell with `min-width: 0`, `min-height: 0`, scrollable side panels, wrapping button rows, and fixed tile dimensions so labels, icons, loading states, and hover styles cannot shift the layout.
- On mobile, collapse side panels below the preview, cap effect-list height, and make buttons wrap cleanly. Text must not overlap, overflow buttons, or cover the camera.
- Before finalizing, run the project build and visually inspect the page at desktop and mobile widths (or use browser screenshots when available). Verify the preview is not blank/cropped incorrectly, controls are reachable, and UI elements do not overlap.
